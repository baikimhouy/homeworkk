      ...
      <div class="p-6">
        <div class="flex justify-between items-start mb-2">
          <a href="detail.html?id=${
            product.id
          }" class="text-lg font-semibold text-gray-800 hover:text-emerald-500 transition">${
    product.title
  }</a>
        ...
        <div class="flex items-center justify-between">
          <div>
            ${
              product.discountPercentage > 0
                ? `<span class="text-gray-400 line-through mr-2">$${product.price.toFixed(
                    2
                  )}</span><span class="text-emerald-500 font-bold">$${discountPrice}</span>`
                : `<span class="text-gray-800 font-bold">$${product.price.toFixed(
                    2
                  )}</span>`
            }
          </div>
